...
plot for the average flow gain from msireport attack
'''
...
for fileid in range(1,25):
    '''
    if(os.path.exists('res3/misreport_0_'+str(fileid))):
        print("---------")
    else:
        continue
    '''
    ...
    for batchid in range(1,4):
        #honest case
        f = open('res'+str(batchid)+'/flowmisreport_0_'+str(fileid), 'rb')
        ...
        #trivial case
        f = open('res'+str(batchid)+'/flowmisreport_1_'+str(fileid), 'rb')
        ...
        #stealthy case
        f = open('res'+str(batchid)+'/flowmisreport_2_'+str(fileid), 'rb')
...
plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.12), ncol=4, prop={'size':10})
plt.grid(linestyle=':')
plt.ylabel("flow/%", fontsize = 14)
plt.xlabel("trace id", fontsize = 15)
plt.savefig("avgflow.eps")
