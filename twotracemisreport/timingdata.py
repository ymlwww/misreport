'''
    test different misreport timing
'''
...
#1. honest case
#2. trivial attack all the time
#3. stealthy attack all the time
#4. stealthy attack with misreport rate 25% (t%4 = 0)
#5. stealthy attack with misreport rate 25% (t%4 = 1)
...
for item in pairdict.keys():
   avg = avg + pairdict[item][5]
avg = avg/len(pairdict.keys())
print(avg)
elephants = {}
for item in pairdict.keys():
    if(pairdict[item][5] > avg):
        elephants[item] = pairdict[item]
f = open('res/pairdict'+str(fileid)+'_p_'+str(percent),'wb')
pickle.dump(elephants, f, pickle.HIGHEST_PROTOCOL)
f.close()

ans = np.zeros(6)
ans[5] = len(pairdict.keys())
for item in pairdict.keys():
    ...
    if pairdict[item][4]>0:
        ans[4] = ans[4] + 1
f = open('res/paircnt'+str(fileid)+'_p_'+str(percent),'wb')
