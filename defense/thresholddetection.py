...
y4= []
y5 =[]
print(collectload0[-1], collectload1[-1], collectload2[-1])
for i in range(int(loadarr[0]),15*int(loadarr[0]),1000):
    t1 = 0
    #print(i)
    for x in loadhist:
        if x<=i:
    ...
    if i<= collectload1[-1]:
        ...
        a = t1/len(loadhist)
        b = 0.24*(i-collectload1[0])/(collectload1[-1] - collectload1[0])
    else:
        y3.append(1)
        a = t1/len(loadhist)
    if i<= collectload2[-1]:
        ...
        a = t1/len(loadhist)
        b = 0.24*(i-collectload2[0])/(collectload2[-1] - collectload2[0])
    else:
        y4.append(1)
        a = t1/len(loadhist)
    y5.append(1)
    #print(t1/len(loadhist))
...
plt.savefig(str(fileid)+"detection.eps")
plt.close()
